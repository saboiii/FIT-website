...
export function segmentBody(body, toJson) {
  ...
  for (const child of [...body.childNodes]) {
    ...
    if (child.hasAttribute?.('data-html-block')) {
      // A rendered-article wrapper (e.g. pasted from the live page source):
      // unwrap it so round trips never nest wrappers inside wrappers.
      flush()
      nodes.push({ type: 'htmlBlock', attrs: { html: child.getAttribute('data-html') || child.innerHTML || '' } })
    } else if (isSimpleBlock(child)) {
...
/**
 * The article's full HTML straight from the doc JSON: htmlBlock nodes
 * contribute attrs.html VERBATIM (no serialize/unescape round trip, which
 * browsers and server DOMs disagree on), runs of normal nodes go through the
 * provided generateHTML. This is what the editor's whole-article code view
 * shows; feeding it back through segmentBody restores an equivalent doc.
 */
export function docToArticleHtml(doc, generateHtml) {
  const parts = []
  let run = []
  const flush = () => {
    if (run.length === 0) return
    parts.push(generateHtml({ type: 'doc', content: run }))
    run = []
  }
  for (const node of doc?.content || []) {
    if (node?.type === 'htmlBlock') {
      flush()
      parts.push(String(node.attrs?.html ?? ''))
    } else if (node) {
      run.push(node)
    }
  }
