...
export function estimatePrintHoursLayerStack(
  ...
  const components = layerStackComponents({ positions, sourceUnit, settings }, model)
  if (!components) return 0
  return hoursFromLayerStackComponents(components, model)
}

/**
 * The estimator's shape-derived inputs, exposed so the validation harness can
 * refit `flowMm3PerS`/`perLayerOverheadS` against real print times without
 * re-slicing per candidate constant (time is linear in these components).
 * @returns {{extrudedMm3: number, totalLayers: number, supportOn: boolean}|null}
 */
export function layerStackComponents(
  { positions, sourceUnit = 'mm', settings = {} } = {},
  model = DEFAULT_LAYER_STACK_MODEL,
) {
  ...
  return { extrudedMm3, totalLayers, supportOn: !!enableSupport }
}

/**
 * Merge admin-fitted constants (AppSettings.quotingConfig.layerStackModel)
 * over the defaults. Null/absent/non-positive values fall back per-field.
 */
export function resolveLayerStackModel(stored) {
  const model = { ...DEFAULT_LAYER_STACK_MODEL }
  if (stored?.flowMm3PerS > 0 && Number.isFinite(stored.flowMm3PerS)) {
    model.flowMm3PerS = stored.flowMm3PerS
  }
  if (stored?.perLayerOverheadS >= 0 && Number.isFinite(stored.perLayerOverheadS)) {
    model.perLayerOverheadS = stored.perLayerOverheadS
  }
  return model
}

/** Compose hours from components under a given (possibly refitted) model. */
export function hoursFromLayerStackComponents(
  { extrudedMm3, totalLayers, supportOn },
  model = DEFAULT_LAYER_STACK_MODEL,
) {
  let seconds = extrudedMm3 / model.flowMm3PerS + totalLayers * model.perLayerOverheadS
  if (supportOn) seconds *= model.supportTimeFactor
