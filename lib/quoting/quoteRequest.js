 ...
 *   abusive magnitudes.
 * - Only geometry METRICS are accepted (not raw mesh data). NOTE: those metrics
 *   are still client-computed; server-side recomputation from the stored model
 *   is a follow-up — see openspec change `add-server-side-geometry-verification`.
 */
import { z } from 'zod'
...
export function buildQuote(rawInput, { pricingConfig = {}, deliveryTypes = [] } = {}) {
  ...
  return { ok: true, status: 200, data: { quote, requestId: input.requestId ?? null } }
}
